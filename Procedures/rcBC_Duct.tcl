...
#
# Description:
# This script contains is a function that create a 3D ductile beam column element with lumped plasticity.
#
# The numerical model herein aims at accurately representing the full range of behavior – from initial cracking
# up through strength and stiffness deterioration behavior that leads to global sidesway collapse.
# The plastic hinge model combined with the moment-curvature analysis procedure are able to capture
# the strength and stiffness deterioration and collapse behavior.
# The plastic hinge model lumps the bond-slip and beam-column yielding response into one concentrated hinge.
# The plastic hinge model for beam-columns have a trilinear backbone curve. This model was originally developed
# by Ibarra et al. (2005, 2003) and implemented in OpenSees by Altoontash (2004). The negative branch of
# the post-peak response simulates strain-softening behaviour associated with phenomena such as concrete
# crushing and rebar buckling and fracture. The accuracy of the onset and slope of this negative branch are
# among the most critical aspects of collapse modelling (Ibarra et al. 2005, and 2003; Haselton et al, 2006).
# The model captures four modes of cyclic degradation: strength deterioration of the inelastic strain hardening
# branch, strength deterioration of the post-peak strain softening branch, accelerated reloading stiffness
# deterioration, and unloading stiffness deterioration. The cyclic deterioration is based on an energy index
# that has two parameters that reflect the normalized energy dissipation capacity and the rate of cyclic deterioration.
# Model parameters for RC beam-columns are based on two sources. The first source consists of empirical relationships
# developed by Fardis et al. (2003, 2001) to predict chord rotation of RC elements at both the yield rotation, and
# at the ultimate rotation where “ultimate” is defined as a reduction in load resistance of at least 20% under monotonic or cyclic loading.
#
...
# -----------------------------------------------------------------------------------------
# -- Script of the Actual Material Model and Element Creation 											 ------
# -----------------------------------------------------------------------------------------
# Ductile Element based on Hasleton Parameters
proc rcBC_Ductile {ET GT iNode jNode fyL fyV Es fc Ec b h s cv dbL dbV P Ls rho_shr rho_top1zz rho_mid1zz rho_bot1zz rho_top2zz rho_mid2zz rho_bot2zz rho_top1yy rho_mid1yy rho_bot1yy rho_top2yy rho_mid2yy rho_bot2yy pfile {pflag 0}} {
global MPa
global mm
# --------------------------------------
