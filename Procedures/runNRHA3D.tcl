...
# analysis as a collapse case. It monitrs the current deformation of a number
...
proc runNRHA3D {Dt Tmax Dc tNode bNode log {pflag 1}} {
	...
	set 	tolInit 	1.0e-4;		# Set the initial tolerance, so it can be referred back to
	...
	while {$cIndex==0 && $controlTime <= $Tmax && $ok==0} {
		...
		if {$ok != 0} {
			puts " ~~~ Failed at $controlTime - Trying Newton With Line Search......"
		...
		if {$ok != 0} {
			puts " ~~~ Failed at $controlTime - Trying Newton With LineSearch & Relaxed Convergence......"
			test NormDispIncr [expr $tolInit*0.1] [expr $iterInit*50]
			algorithm NewtonLineSearch .8
			set ok [analyze 1 $Dt]
			test 		$testType $tolInit $iterInit
		...
		if {$ok != 0} {
			puts " ~~~ Failed at $controlTime - Trying Newton With Line Search, Reduced Timestep & Relaxed Convergence......"
			test NormDispIncr [expr $tolInit*0.1] [expr $iterInit*50]
			algorithm NewtonLineSearch .8
			set Dtt [expr 0.5*$Dt]
			set ok [analyze 1 $Dtt]
